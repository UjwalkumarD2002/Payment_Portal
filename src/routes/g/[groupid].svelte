			...
			onClick: () => {
				if (navigator.share) {
					navigator
						.share({
							title: 'SplitPay',
							text: "Let's split bills with this group in SplitPay 💰",
...
<SvelteSeo
	openGraph={{
		title: 'SplitPay | group',
		description:
			'split your bills easily! SplitPAy is an open-source webapp built for tracking debts and payments quickly, without any user accounts.',
...
<svelte:head>
	<title>SplitPay | {$groupStore.groupInfo.name}</title>
</svelte:head>
